    ...
    public class BuildingToolWatcher : ThreadingExtensionBase
    {
        ...
        private void Apply(BuildingInfo prefab)
        {
            ...
            foreach (var networkDef in buildingDef.NetworkDefs)
            {
                ...
                if (networkDef.PillarProp != null)
                {
                    PropInfo PillarProp = null;
                    if (networkDef.PillarProp != NoneValue)
                        PillarProp = PrefabCollection<PropInfo>.FindLoaded(networkDef.PillarProp);
                    data.SetPrefabFeature(SegmentData.FeatureFlags.PillarProp, PillarProp);
                }
