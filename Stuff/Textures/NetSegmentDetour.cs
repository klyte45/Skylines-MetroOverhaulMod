    ...
    public struct NetSegmentDetour
    {
        ...
        private void RenderInstance(RenderManager.CameraInfo cameraInfo, ushort segmentID, int layerMask, NetInfo info, ref RenderManager.Instance data)
        {
            ...
            if (data.m_dirty)
            {
                ...
                if ((_this.m_flags & NetSegment.Flags.NameVisible2) != NetSegment.Flags.None)
                {
                    ...
                    if (data.m_nameData != null)
                    {
                        ...
                        Vector3 middlePos;
                        Vector3 middlePos2;
                        NetSegment.CalculateMiddlePoints(position, _this.m_startDirection, position2, _this.m_endDirection, true, true, out middlePos, out middlePos2);
            ...
            if (info.m_lanes != null && ((layerMask & info.m_propLayers) != 0 || cameraInfo.CheckRenderDistance(data.m_position, info.m_maxPropDistance + 128f)))
            {
                ...
                if ((_this.m_flags & NetSegment.Flags.Collapsed) != NetSegment.Flags.None)
                {
                    int num6 = 0;
