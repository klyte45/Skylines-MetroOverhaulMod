...
namespace MetroOverhaul
{
    public class Initializer : AbstractInitializer
    {
        ...
        #endregion
        private void CreateClonedTracks()
        {
            try
            {
                CreateClonedPrefab(ActionExtensions.BeginChain<NetInfo, NetInfoVersion>().
                    Chain(CustomizationSteps.ExtraPillarsCustomization).
                    Chain(
                            (info, version) =>
                            {
                                LoadingExtension.EnqueueLateBuildUpAction(() => {
                                    LateBuildUpSteel.BuildUp(info, version);
                                    LateBuildUpSteel.BuildUpExtraPillars(info, version);
                                });
                            }),
                    NetInfoVersion.Ground | NetInfoVersion.Elevated | NetInfoVersion.Bridge | NetInfoVersion.Slope | NetInfoVersion.Tunnel,
                    ActionExtensions.BeginChain<NetInfo, Action<NetInfo, NetInfoVersion>>().
                        Chain<NetInfo, Action<NetInfo, NetInfoVersion>, Func<string, string>, NetInfoVersion>(
                            LinkToNonGroundClonedVersions, null, NetInfoVersion.None), "Steel Metro Track Ground NoBar", prefabName => prefabName + " ep");
            }
            catch (Exception e)
            {
                Next.Debug.Log("Exception happened when setting up extra pillar asset from Steel Metro Track Ground NoBar");
                UnityEngine.Debug.LogException(e);
            }
        }
        ...
        protected void CreateClonedPrefab(Action<NetInfo, NetInfoVersion> customizationStep, NetInfoVersion versions,
            Action<NetInfo, Action<NetInfo, NetInfoVersion>> setupOtherVersionsStep,
            string cloneFrom, Func<string, string> nameModifier = null)
        {
            NetInfo groundVersion = null;
            if ((versions & NetInfoVersion.Ground) != 0)
            {
                    groundVersion = CreateClonedNetInfo(nameModifier.Invoke(cloneFrom), cloneFrom,
                    ActionExtensions.BeginChain<NetInfo>().
                    Chain(p =>
                    {
                        setupOtherVersionsStep.Invoke(p, customizationStep);
            ...
            foreach (var version in new[] { NetInfoVersion.Bridge, NetInfoVersion.Tunnel, NetInfoVersion.Elevated, NetInfoVersion.Slope, })
            {
                if ((versions & version) != 0)
                {
                    CreateClonedNetInfo(nameModifier.Invoke(Util.SwitchVersion(cloneFrom, version)), Util.SwitchVersion(cloneFrom, version), 
                        ActionExtensions.BeginChain<NetInfo>().
                        Chain(CommonSteps.SetVersion, groundVersion, version).
                        Chain(SetupTrackModel, customizationStep.Chain(SetCosts)));
                }
        ...
        protected void LinkToNonGroundClonedVersions(NetInfo p, Action<NetInfo, NetInfoVersion> customizationStep,
            Func<string, string> nameModifier = null, NetInfoVersion versions = NetInfoVersion.Slope | NetInfoVersion.Tunnel | NetInfoVersion.Elevated | NetInfoVersion.Bridge)
        {
            if (nameModifier == null)
            {
            ...
            foreach (var version in new[] { NetInfoVersion.Bridge, NetInfoVersion.Tunnel, NetInfoVersion.Elevated, NetInfoVersion.Slope, })
            {
                ...
                CommonSteps.SetVersion(FindCustomNetInfo(nameModifier.Invoke(Util.SwitchVersion("Steel Metro Track Elevated NoBar", version))), p, version);
            }
        }
